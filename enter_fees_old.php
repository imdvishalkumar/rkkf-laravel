...
if($submitClick) {
    ...
    while ($selectedmonth > 0) {
        ...
        if ( $temp == 0 ) {
            ...
            $feeQuery .= "INSERT INTO fees (student_id,months,year,date,amount,coupon_id,additional,disabled,mode,remarks) SELECT $id,".$month.",$year,'$currentDate','".( $fees + $check )."',1,0,0,'cash','$remarks' where (SELECT COUNT(*) FROM fees WHERE student_id = $id AND months = ".$month." AND year = $year) = 0 ;";
            ...
            $feeQuery .= "INSERT INTO fees (student_id,months,year,date,amount,coupon_id,additional,disabled,mode,remarks) SELECT $id,".$month.",$year,'$currentDate','$fees',1,0,0,'cash','$remarks' where (SELECT COUNT(*) FROM fees WHERE student_id = $id AND months = ".$month." AND year = $year) = 0 ;";
        }
        if ($month == 12){
            $month = 0;
            $year = $year +1;
...
<body class="sidebar-mini layout-fixed" style="height: auto;">
    ...
    <script>
        ...
        function get_studentInfo() { // Call to ajax function
            ...
            $.ajax({
                type: "POST",
                url: "enter_fees_old_ajax.php", // Name of the php files
                data: dataString,
