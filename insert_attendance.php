...
if($submitClick) {
    ...
    foreach($attenArray as $index => $value){
        $query = "INSERT INTO attendance (student_id, date, attend, branch_id, user_id, is_additional) SELECT * FROM (SELECT '".$attenIDs[$index]."' AS student_id, '".$date."' AS date, '".$attenArray[$index]."' AS attend, '".$ename."' AS branch_id, '0' AS user_id, '0' AS is_additional) AS tmp WHERE NOT EXISTS ( SELECT attendance_id FROM attendance WHERE student_id = '".$attenIDs[$index]."' AND date = '".$date."' AND branch_id = '".$ename."' ) LIMIT 1;";
        // $query = "INSERT INTO `attendance` (`attendance_id`, `student_id`, `date`, `attend`, `branch_id`, `user_id`, `is_additional`) VALUES (NULL, ".$attenIDs[$index].", '".$date."', '".$attenArray[$index]."', '".$ename."', '0', '0')";
...
<body class="sidebar-mini layout-fixed" style="height: auto;">
    ...
    <script type="text/javascript">
        function passId(val) {
            $("#exam_id").val(val); //set the id to the input on the modal
        }
    ...
    <script>
        function onBranchChange() { // Call to ajax function
            ...
            $.ajax({
                type: "POST",
                url: "insert_attendance_ajax.php", // Name of the php files
                data: {
                    branchId: branchId
                },
