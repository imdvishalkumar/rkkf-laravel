...
if ( $submitClick ) {
    ...
    foreach($variations as $index => $value){
        $fees = ((100/90)*((int)$prices[$index]));
        $rounded = ceil($fees / 10) * 10;

        $query = "INSERT INTO variation (product_id,variation,price,qty) VALUES ('".$last_id."','".$variations[$index]."','".$rounded."','".$qtys[$index]."')"; 
