...
if(isset($_GET['event_id'])) {
    ...
    $sql = "SELECT s.student_id, e.event_id, CONCAT(s.firstname, ' ', s.lastname) as name, b.name as branch_name, (SELECT name FROM belt WHERE belt_id = s.belt_id) as current_belt, IFNULL((SELECT DISTINCT ex.date FROM exam ex, exam_attendance ea WHERE ea.student_id = s.student_id AND ex.exam_id = ea.exam_id AND ea.attend = 'P' AND ex.isPublished = 1 ORDER BY ex.date DESC LIMIT 1),'Not Found') as last_exam_date ,IFNULL((SELECT DISTINCT eligible FROM special_case_event WHERE event_id = e.event_id AND student_id = s.student_id),0) as atten_eligible FROM students s, branch b, event e WHERE b.branch_id = s.branch_id AND s.active = 1 AND e.event_id = '".$eventId."'";
...
<body class="sidebar-mini layout-fixed" style="height: auto;">
  ...
  while($row = $result->fetch_assoc()) {
    echo "<tr>  <td>" . $row["student_id"]. "</td>
          ...
          if($row["atten_eligible"] == '0') {
              echo "<button id='btnModal' value='".$row['student_id']."' onclick='eligibleStudent(".$row['student_id'].",".$row['event_id'].",true)' style='padding: 0;border: none;background: none;'><span class='fas fa-check'></span></button>";
          }	else {
              echo "<button id='btnModal' value='".$row['student_id']."' onclick='eligibleStudent(".$row['student_id'].",".$row['event_id'].",false)' style='padding: 0;border: none;background: none;'><span class='fas fa-times'></span></button>";
          }
		echo"	
		  </div>
          </td>
...
<script>
    ...
    <script type="text/javascript">
        ...
        function eligibleStudent(studentId, examId, eligible) {
            ...
            $.ajax({
                    type: "POST",
                    url: "eligible_students_event_ajax.php", // Name of the php files
                    data: {
