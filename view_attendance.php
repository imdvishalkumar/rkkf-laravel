...
$resultForBranch = $con->query($queryForBranch);
    ...
    $sql = "SELECT a.*,CONCAT(s.firstname,' ',s.lastname) as name, (SELECT name from branch WHERE branch_id = a.branch_id) as branch_name, (SELECT CONCAT(firstname,' ',lastname) from users WHERE user_id = a.user_id) as ins_name FROM attendance a, students s WHERE s.active = 1 AND a.student_id = s.student_id LIMIT 1000";
