  ...
  while($row = $result->fetch_assoc()) {
    echo "<tr id='".$row['fee_id']."'>  
        <td>
          ...
          echo "</td>
          <td>" . '<a href="javascript:void(0);" class="btn btn-success changeFeeCallFlag '.$setClass.'" data-id="'.$row["fee_id"].'">'.$str.'</a>' . "</td>
		  </tr>";
  }
} else {
 ?><tr>
    ...
    <script>
        ...
        function deleteFee() { // Call to ajax function
            ...
            $.ajax({
                type: "POST",
                url: "delete_fees_ajax.php", // Name of the php files
                data: {
                    fee_id: feeId
                },
                dataType: 'JSON',
        ...
        $(document).on("click",".changeFeeCallFlag",function() {
            var feesId = $(this).attr('data-id');
            swal({
                title: "Change Flag",
                text: "Are you sure ? You want to change flag for this particular fees ?  ",
                type: "warning",
